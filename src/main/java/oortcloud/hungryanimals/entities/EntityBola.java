...
public class EntityBola extends Entity implements IProjectile {
	...
	public EntityBola(EntityType<?> entityTypeIn, World world) {
		super(entityTypeIn, world);
	}

	public EntityBola(World world, double posx, double posy, double posz) {
		this(ModEntities.BOLA.get(), world);
	...
	public void onUpdate() {
		...
		if (this.isInWater())
        {
            for (int i = 0; i < 4; ++i)
            {
                this.getEntityWorld().addParticle(EnumParticleTypes.WATER_BUBBLE, this.posX - this.motionX * 0.25D, this.posY - this.motionY * 0.25D, this.posZ - this.motionZ * 0.25D, this.motionX, this.motionY, this.motionZ, new int[0]);
            }
