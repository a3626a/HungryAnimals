...
public class EntityAIAvoidPlayer extends EntityAIBase {
	...
	public boolean shouldExecute() {
		...
		if (!(players != null && !players.isEmpty() && property.taming < -1)&&!(this.entity.getAITarget()!=null)) {
			...
			for (Object i : players) {
				EntityLivingBase p = ((EntityLivingBase)i);
			...
			if (target == null) return false;
				...
				target=this.entity.getAITarget();
				players.add(entity);
			}
			...
			Vec3 vec3 = RandomPositionGenerator.findRandomTargetBlockAwayFrom(
					this.entity, 16, 8, new Vec3(target.posX, target.posY, target.posZ));
	...
	public void startExecuting() {
		this.entity.getNavigator().tryMoveToXYZ(this.xPosition, this.yPosition,
				this.zPosition, this.speed*(this.entity.getAITarget()==null?1:1.5));
	}
