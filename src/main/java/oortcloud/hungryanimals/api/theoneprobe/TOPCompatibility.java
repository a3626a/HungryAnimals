            ...
            probe.registerEntityProvider(new IProbeInfoEntityProvider() {
				...
				public void addProbeEntityInfo(ProbeMode mode, IProbeInfo probeInfo, EntityPlayer player, World world, Entity entity, IProbeHitEntityData data) {
					...
					if (capTaming.getTaming() >= 0) {
						probeInfo.horizontal().text("TAMING").progress((int)capTaming.getTaming(), 2, probeInfo.defaultProgressStyle().filledColor(0xFF00FF00).alternateFilledColor(0xFF00FF00).borderColor(0));
					} else {
						probeInfo.horizontal().text("TAMING").progress((int)(-capTaming.getTaming()), 2, probeInfo.defaultProgressStyle().filledColor(0xFFFF0000).alternateFilledColor(0xFFFF0000).borderColor(0));
					}
				}
			});
            return null;
